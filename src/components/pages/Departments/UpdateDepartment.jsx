...
const UpdateDepartment = () => {
    ...
    const fetchDepartmentData = async () => {
        try {
            const response = await axiosInstance.get(`/v1/admin/departments/${departmentId}`);
            if (response.data && response.data.data) {
                const department = response.data.data;
                setFormData({
                    name: department.translations.name,
    ...
    const validateForm = () => {
        ...
        if (formData.year <= 0) newErrors.year = "سنة البداية  مطلوب";
        setErrors(newErrors);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axiosInstance.put(`/v1/admin/departments/${departmentId}`, formData);
                            ...
                            <div className="row align-items-center">
                                <div className="col-sm-12">
                                    <div className="page-sub-header">
                                        <h3 className="page-title">تحديث القسم</h3>
                                        <ul className="breadcrumb">
                                            <li className="breadcrumb-item">
                                                <Link to={`/admin/faculties/${facultyId}/departments`}>الأقسام</Link>
                                            </li>
                                            <li className="breadcrumb-item active">تحديث القسم</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <form onSubmit={handleSubmit}>
                                            <div className="row">
                                                ...
                                                <div className="col-12 col-sm-6">
                                                    <div className="form-group local-forms">
                                                        <label>
                                                             سنة البداية <span className="login-danger">*</span>
                                                        </label>
                                                        <input
                                                            className="form-control"
                                                            type="number"
