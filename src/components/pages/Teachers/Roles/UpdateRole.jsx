...
const UpdateTeacherRole = () => {
  ...
  useEffect(() => {
    if (roleData) {
      setFormData({
        name: roleData.name,
        readable_name: {
          en: roleData.translations.readable_name.en,
          ar: roleData.translations.readable_name.ar
        },
  ...
  const fetchRole = async () => {
    try {
      const response = await axiosInstance.get(`/v1/admin/roles/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        name: formData.name,
        readable_name: formData.readable_name,
...
export default UpdateTeacherRole;
