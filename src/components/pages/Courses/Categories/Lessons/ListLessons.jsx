...
const truncateDescription = (htmlContent, maxLength) => {
    const plainText = parse(htmlContent);
...
const ListLessons = () => {
    ...
    const [quizPagination, setQuizPagination] = useState({ current: 1, pageSize: 10, total: 0 }); // New state for quiz pagination
    const [loadingLessons, setLoadingLessons] = useState(false);
    const [loadingAssignments, setLoadingAssignments] = useState(false);
    const [loadingQuizzes, setLoadingQuizzes] = useState(false); // New state for loading quizzes
    ...
    useEffect(() => {
        ...
        fetchQuizzes(quizPagination.current, quizPagination.pageSize); // Fetch quizzes data
    }, []);
    ...
    const fetchQuizzes = async (page, limit) => {
        ...
        try {
            const response = await axiosInstance.get(`v1/admin/courses/${courseId}/course-content-categories/${categoryId}/quizzes?limit=${limit}&page=${page}`);
            const data = response.data;
            if (Array.isArray(data.data.items)) {
                setQuizzes(data.data.items);
                setQuizPagination(prev => ({ ...prev, total: data.data.total }));
            } else {
                console.error('API response is not an array', data.data);
            }
        } catch (error) {
            console.error('Error fetching quizzes:', error);
            setError("حدث خطأ أثناء جلب بيانات الاختبارات. الرجاء المحاولة لاحقاً.");
        } finally {
    ...
    const handleDeleteQuiz = async (quizId) => {
        Modal.confirm({
            title: 'هل انت متأكد بأنك تريد حذف هذا الاختبار',
            content: 'يمكنك عدم تنفيذ هذا',
            okText: 'حذف',
            okType: 'danger',
            cancelText: 'تراجع',
            onOk: async () => {
                try {
                    await axiosInstance.delete(`v1/admin/courses/${courseId}/course-content-categories/${categoryId}/quizzes/${quizId}`);
                    setQuizzes(prevQuizzes => prevQuizzes.filter(quiz => quiz.id !== quizId));
                    message.success('تم حذف الاختبار بنجاح');
                } catch (error) {
                    setError("فشل في حذف هذا الاختبار");
                }
            }
        });
    };
    ...
    const handleQuizTableChange = (pagination) => {
    ...
    const lessonColumns = [
        ...
        },
        {
            ...
            key: "description",
            render: (text, record) => (
                <span>{truncateDescription(record.translations.description.ar, MAX_DESCRIPTION_LENGTH)}</span>
            )
        },
        {
            title: "تاريخ الإنشاء",
    ...
    const assignmentColumns = [
        ...
        },
        {
            ...
            key: "description",
            render: (text, record) => <span>{truncateDescription(record.description, MAX_DESCRIPTION_LENGTH)}</span>
        },
        {
    ...
    const quizColumns = [
        ...
        },
        {
            ...
            render: (text, record) => (
                <div className="actions">
                    {CheckPermission("view_quiz") &&
                        <Link to={`/admin/faculties/${facultyId}/courses/${courseId}/categories/${categoryId}/lessons/${record.id}/show-quiz`} className="btn btn-sm btn-rounded bg-primary-light me-2 rounded-full" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <FeatherIcon icon="eye" size="16" />
                        </Link>
                    }
                    {CheckPermission("edit_quiz") &&
                        <Link to={`${record.id}/edit-quiz`} className="btn btn-sm bg-success-light me-2 rounded-full" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <FeatherIcon icon="edit" size="16" />
                        </Link>
                    }
                    {CheckPermission("delete_quiz") &&
                        <Button onClick={() => handleDeleteQuiz(record.id)} className="btn btn-sm bg-danger-light me-2 rounded-full" style={{ padding: '2px 5px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <FeatherIcon icon="trash" size="16" />
                        </Button>
                    }
                </div>
                            ...
                            <div className="card card-table comman-shadow">
                                <div className="card-body">
                                    <div className="page-header">
                                        <div className="row align-items-center">
                                            <div className="col">
                                                <h3 className="page-title">الاختبارات</h3>
                                            </div>
                                            <div className="col-auto text-end float-end ms-auto download-grp">
                                                {CheckPermission("create_quiz") && (
                                                    <Link to={`create-quiz`} className="btn btn-primary">
                                                        <FeatherIcon icon="plus" /> إضافة اختبار
                                                    </Link>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <Table
                                            pagination={{
                                                ...
                                                showTotal: (total, range) =>
                                                    `عرض ${range[0]} إلى ${range[1]} من ${total} مدخلات`,
                                                showSizeChanger: true,
                                                onShowSizeChange: (current, size) => handleQuizTableChange({ current, pageSize: size }),
                                                itemRender: itemRender,
                                            ...
                                            dataSource={quizzes}
                                            rowKey={(record) => record.id}
                                            onChange={handleQuizTableChange}
