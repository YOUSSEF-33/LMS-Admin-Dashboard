...
const UpdateQuiz = () => {
    const { courseId, categoryId, quizId } = useParams();
    ...
    const fetchQuizData = async () => {
        try {
            const response = await axiosInstance.get(`v1/admin/courses/${courseId}/course-content-categories/${categoryId}/quizzes/${quizId}`);
            const quizData = response.data.data;

            let validatedStartAt = null;
            let validatedDeadLine = null;
            if (quizData.start_at) {
                const parsedStartDate = moment(quizData.start_at);
                if (parsedStartDate.isValid()) {
                    validatedStartAt = parsedStartDate.toDate();
                } else {
                    console.error("Invalid start date format received from backend");
                    message.error("Invalid start date received from the server");
                }
            }
            if (quizData.dead_line) {
                const parsedEndDate = moment(quizData.dead_line);
                if (parsedEndDate.isValid()) {
                    validatedDeadLine = parsedEndDate.toDate();
                } else {
                    console.error("Invalid deadline date format received from backend");
                    message.error("Invalid deadline date received from the server");
                }
            }

            const questionPromises = quizData.questions.map(async q => {
            ...
            setFormData({
                ...
                groups: quizData.groups.map(group => group.id),
                total_marks: quizData.total_marks,
                start_at: validatedStartAt,
    ...
    const validateForm = () => {
        const newErrors = {};
        if (!formData.title) newErrors.title = "عنوان الاختبار مطلوب";
        if (formData.groups.length === 0) newErrors.groups = "يجب اختيار مجموعة واحدة على الأقل";
        if (!formData.start_at) newErrors.start_at = "تاريخ البدء مطلوب";
    ...
    const handleSubmit = async (e) => {
        ...
        formDataToSend.append('start_at', formData.start_at?.getTime());
        ...
        try {
            await axiosInstance.post(`v1/admin/courses/${courseId}/course-content-categories/${categoryId}/quizzes/${quizId}`, formDataToSend, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
            message.success("تم تحديث الاختبار بنجاح");
            navigate("/admin/quizzes");
        } catch (error) {
            console.error("Error updating quiz:", error);
            message.error("حدث خطأ أثناء تحديث الاختبار");
        }
    };
                    ...
                    <div className="row align-items-center">
                        <div className="col-sm-12">
                            <div className="page-sub-header">
                                <h3 className="page-title">تحديث الاختبار</h3>
                                <ul className="breadcrumb">
                                    <li className="breadcrumb-item">
                                        <Link to="/quizzes">الاختبارات</Link>
                                    </li>
                                    <li className="breadcrumb-item active">تحديث الاختبار</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                                ...
                                <form onSubmit={handleSubmit}>
                                    <div className="row">
                                        <div className="col-12">
                                            <h5 className="form-title student-info">معلومات الاختبار <span><FeatherIcon icon="more-vertical" /></span></h5>
                                        </div>
                                        <div className="col-12">
                                            <div className="form-group local-forms">
                                                <label>العنوان <span className="login-danger">*</span></label>
                                                <input type="text" name="title" className="form-control" placeholder="أدخل عنوان الاختبار" value={formData.title} onChange={handleInputChange} />
                                        ...
                                        <div className="col-12 col-sm-6">
                                            <div className="form-group local-forms">
                                                <label>تاريخ البدء <span className="login-danger">*</span></label>
                                                <DatePicker
                                                ...
                                                />
                                                {errors.start_at && <div className="text-danger">{errors.start_at}</div>}
                                            </div>
                                        </div>
                                        <div className="col-12 col-sm-6">
                                            <div className="form-group local-forms">
                                        ...
                                        <div className="col-12">
                                            <div className="student-submit">
                                                <button type="submit" className="btn btn-primary">تحديث الاختبار</button>
                                            </div>
                                        </div>
                                    </div>
                                </form>
