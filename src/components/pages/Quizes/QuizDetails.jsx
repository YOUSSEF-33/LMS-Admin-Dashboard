...
const QuizDetails = () => {
    const { id, courseId, categoryId, quizId } = useParams();
    const facultyId = id;
    ...
    const fetchQuizDetails = async () => {
        ...
        try {
            ...
            setError("حدث خطأ أثناء جلب بيانات الاختبار. الرجاء المحاولة لاحقاً.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchSubmissions = async (page = 1, pageSize = 10) => {
        ...
        try {
            const response = await axiosInstance.get(`v1/admin/courses/${courseId}/course-content-categories/${categoryId}/quizzes/${quizId}/submissions`, {
    ...
    const handleShowGrades = async () => {
        try {
            await axiosInstance.patch(`v1/admin/courses/${courseId}/course-content-categories/${categoryId}/quizzes/${quizId}/show-marks-to-students`);
    ...
    const handleViewSubmission = (record) => {
        navigate(`/admin/faculties/${facultyId}/courses/${courseId}/categories/${categoryId}/quizzes/${quizId}/submissions/${record.id}`,
                ...
                <div className="row align-items-center">
                    <div className="col">
                        ...
                        <ul className="breadcrumb">
                            ...
                            <li className="breadcrumb-item"><Link to={`/admin/faculties/${facultyId}/courses/${courseId}/categories/${categoryId}/lessons`}>الاختبارات</Link></li>
                            <li className="breadcrumb-item active">تفاصيل الاختبار</li>
                        </ul>
                    </div>
                </div>
                <div className='mt-5'>
                    <div className="row">
                        <p className="text-muted">
                            <strong>تاريخ البدء:</strong> {moment(quiz.start_at).format('YYYY-MM-DD HH:mm:ss')}
                        </p>
                        <p className="text-muted">
                            <strong>تاريخ الانتهاء:</strong> {moment(quiz.dead_line).format('YYYY-MM-DD HH:mm:ss')}
                        </p>
                        <p className="text-muted">
                            <strong>الدرجات الكلية:</strong> {quiz.total_marks}
                        </p>
                        <p className="text-muted">
                            <strong>عدد الطلاب:</strong> {quiz.students_count}
                        </p>
                        <p className="text-muted">
                            <strong>عدد التسليمات:</strong> {quiz.submit_count}
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div className="section mb-4">
                    <div className="d-flex justify-content-between align-items-center mb-2">
                        ...
                        {CheckPermission("edit_quiz") && (
                            <Link to={`/admin/faculties/${courseId}/courses/${courseId}/categories/${categoryId}/lessons/${quizId}/edit-quiz`} className="btn btn-primary">
                    ...
                    <Row justify="space-between" align="middle" style={{ marginBottom: '20px' }}>
                        ...
                        <Col>
                            <Button
                                ...
                                disabled={quiz.show_marks_to_students === 1}
                            >
                                {quiz.show_marks_to_students === 1 ? 'تم إظهار الدرجات للطلاب' : 'إظهار الدرجات للطلاب'}
                            </Button>
                        </Col>
                    </Row>
                    <Table
